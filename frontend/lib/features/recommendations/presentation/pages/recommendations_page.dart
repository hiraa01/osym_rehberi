      ...
      if (studentId != null) {
        ...
        if (forceRefresh) {
          try {
            ...
            debugPrint(
                '✅ Eski öneriler temizlendi, yeni öneriler hesaplanıyor...');
          } catch (e) {
            debugPrint(
                '⚠️ Eski öneriler temizlenirken hata (devam ediliyor): $e');
          }
        }
        ...
        if (mounted) {
          setState(() {
            // ✅ CRITICAL FIX: Backend formatı {"recommendations": [...]} veya direkt liste
            final data = response.data;
            final listData = (data is Map ? data['recommendations'] : data);

            List<dynamic> newRecommendations = [];
            if (listData != null && listData is List) {
              newRecommendations = List.from(listData);
              debugPrint(
                  '✅ Loaded ${newRecommendations.length} recommendations from API');
            } else {
              debugPrint(
                  '⚠️ Recommendations data is null or not a List: ${listData?.runtimeType}');
            }
          ...
          debugPrint(
              '📍 Önerilerde bulunan şehirler: ${citiesInRecommendations.toList()..sort()}');
        }
      } else {
        if (mounted) {
          setState(() {
                  ...
                  Row(
                    children: [
                            ...
                            return citiesAsync.when(
                              data: (cities) {
                                return OutlinedButton.icon(
                                  onPressed: () =>
                                      _showCityFilter(context, cities),
                                  icon:
                      ...
                      Expanded(
                        child: Consumer(
                          builder: (context, ref, child) {
                            final universityTypesAsync =
                                ref.watch(universityTypeListProvider);
                            return universityTypesAsync.when(
                              data: (types) {
                                return OutlinedButton.icon(
                                  onPressed: () =>
                                      _showUniversityTypeFilter(context, types),
                  ...
                  _filteredRecommendations.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final finalScore = (rec['final_score'] ??
