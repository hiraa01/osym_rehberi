"""
✅ KESİN VERİ AKTARIMI (Hardcoded Columns)

Bu script, YÖK ATLAS Excel dosyalarını SABİT SÜTUN İNDEKSLERİ ile okur ve
veritabanına %100 garantili şekilde aktarır.

SÜTUN HARİTASI (Index 0'dan başlar):
- Col 0: Program Kodu
- Col 1: Program Adı (veya Üniversite Başlığı)
- Col 3: Puan Türü (SAY, EA, SÖZ, DİL, TYT)
- Col 4: Kontenjan
- Col 11: Başarı Sırası
- Col 12: En Küçük Puan (Taban Puan)

KULLANIM:
    python scripts/final_import.py

HEDEF: 21.600+ Satırın hepsi veritabanına girmeli.
"""
import pandas as pd
import os
import re
...
def clean_program_name(name: str) -> str:
    """Program adını temizler - parantez içlerini KORUR."""
    if is_na_value(name):
        return ""
    name = str(name).strip()
...
def determine_duration(program_name: str, degree_type: str) -> int:
    """Süreyi belirler."""
    if degree_type == 'Associate':
        return 2
...
# ---------------------------------------------------------
# 🚀 SABİT SÜTUN İNDEKSLERİ İLE İŞLEME
# ---------------------------------------------------------
def process_file_hardcoded(filepath: str, filename: str) -> List[Dict]:
    """Hiyerarşik Excel dosyasını SABİT SÜTUN İNDEKSLERİ ile işler."""
    ...
    try:
        if filename.endswith('.csv'):
            # CSV için: sep=None, engine='python', dtype=str
            df = pd.read_csv(filepath, sep=None, engine='python', dtype=str, header=None)
    ...
    for idx in range(len(df)):
        ...
        if col0_value and is_numeric_code(col0_value):
            ...
            program_name_original = clean_program_name(program_name_raw)
            normalized_name = program_name_original.lower().strip()
            
            # Diğer bilgileri al (SABİT İNDEKSLER)
            field_type_raw = safe_get_value(row, COL_FIELD_TYPE, None)
            quota_raw = safe_get_value(row, COL_QUOTA, "0")
            rank_raw = safe_get_value(row, COL_RANK, None)
...
def main():
    ...
    print("="*60)
    print("📊 KESİN VERİ AKTARIMI (Hardcoded Columns)")
    print("="*60)
    ...
    for filepath in files:
        ...
        try:
            programs = process_file_hardcoded(filepath, filename)
    ...
    # TEKİLLEŞTİRME: Aynı kodlu kayıtlar varsa (çok nadir) onları temizle
    ...
    if not os.path.exists(seed_script_path):
        print(f"⚠️  seed_db.py not found at {seed_script_path}")
        print("   💡 Please run seed_db.py manually:")
        print(f"   python scripts/seed_db.py --json-file {OUTPUT_FILE}")
        return
    ...
    try:
        ...
        # seed_db.py backend_dir'den başlayarak path bekliyor
        # OUTPUT_FILE zaten backend_dir/data/final_cleaned_data.json formatında olmalı
        # Relative path oluştur
        json_file_arg = os.path.relpath(OUTPUT_FILE, backend_dir)
        
        # Windows path'lerini düzelt
        json_file_arg = json_file_arg.replace('\\', '/')
        
        cmd = [python_cmd, seed_script_path, '--json-file', json_file_arg, '--truncate']
        
        print(f"🚀 Running: {' '.join(cmd)}")
        print(f"📂 Working directory: {backend_dir}")
        print(f"📄 JSON file: {json_file_arg}")
        ...
        if result.returncode == 0:
            ...
            print(f"❌ Seed işlemi hatası (return code: {result.returncode}):")
            if result.stderr:
                print(result.stderr)
            if result.stdout:
                print(result.stdout)
            ...
            print(f"   python scripts/seed_db.py --json-file {json_file_arg} --truncate")
    except Exception as e:
        logger.error(f"Error running seed_db.py: {e}", exc_info=True)
        print(f"❌ Seed script çalıştırma hatası: {e}")
        print("\n💡 Manuel olarak çalıştırabilirsiniz:")
        json_file_arg = os.path.relpath(OUTPUT_FILE, backend_dir).replace('\\', '/')
        print(f"   cd {backend_dir}")
        print(f"   python scripts/seed_db.py --json-file {json_file_arg} --truncate")
